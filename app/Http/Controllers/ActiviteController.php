...
class ActiviteController extends Controller
{
    public function index(Request $request)
    {
        ...
        $totalAbonnesActifs = DB::table('subscriptions')->where('statut', 'actif')->count();
    ...
    public function getData(Request $request)
    {
        ...
        $capacite = $request->input('filters.capacite');

        $baseQuery = Service::where('type', 'activite');
        $query = (clone $baseQuery)->with('coach')
        ...
        if ($capacite === 'pleine') {
            $query->whereNotNull('capacite_max')
                ->havingRaw('subscriptions_count >= capacite_max');
        } elseif ($capacite === 'disponible') {
            $query->where(function ($q) {
                $q->whereNull('capacite_max')
                    ->orWhere('capacite_max', 0)
                    ->orWhereRaw('subscriptions_count < capacite_max');
            });
        }
        ...
        $activites = $query->skip($start)->take($length)->orderBy('created_at', 'desc')->get();

        $data = [];
        foreach ($activites as $index => $activite) {
            $capaciteMax = (int) ($activite->capacite_max ?? 0);
            $subscriptionsCount = (int) ($activite->subscriptions_count ?? 0);
            $capacityLabel = $capaciteMax > 0 ? $subscriptionsCount . ' / ' . $capaciteMax : $subscriptionsCount . ' / Illimite';
            $capacityPercent = $capaciteMax > 0 ? min(100, ($subscriptionsCount / max($capaciteMax, 1)) * 100) : 0;
            $capacityBarClass = $capaciteMax > 0 && $subscriptionsCount >= $capaciteMax ? 'bg-danger' : 'bg-success';
            ...
            $data[] = [
                ...
                'prix' => '<div class="text-center"><div>M: ' . $activite->prix_mensuel . ' DH</div><div>T: ' . $activite->prix_trimestriel . ' DH</div><div>A: ' . $activite->prix_annuel . ' DH</div></div>',
                'capacite' => '<div class="text-center"><div>' . $capacityLabel . '</div><div class="progress" style="height: 5px;"><div class="progress-bar ' . $capacityBarClass . '" style="width: ' . $capacityPercent . '%"></div></div></div>',
                'statut_badge' => $activite->statut == 'actif'
                    ? '<span class="badge badge-success">Actif</span>'
                    : '<span class="badge badge-danger">Inactif</span>',
                'action' => '<div class="btn-group btn-group-sm"><button class="btn btn-info view-btn" data-id="' . $activite->id . '" title="Voir"><i class="fas fa-eye"></i></button><button class="btn btn-warning edit-btn" data-id="' . $activite->id . '" title="Editer"><i class="fas fa-edit"></i></button><button class="btn btn-danger delete-btn" data-id="' . $activite->id . '" title="Supprimer"><i class="fas fa-trash"></i></button></div>',
            ];
        }
    ...
    public function destroy(Service $activite)
    {
        try {
            ...
            if ($subscriptionsActives > 0) {
                return response()->json([
                    'success' => false,
                    'message' => 'Impossible de supprimer: activite a des subscriptions actives',
                ], 400);
            }
    ...
    public function export(Request $request)
    {
        ...
        $callback = function () use ($activites) {
            ...
            fputcsv($file, ['ID', 'Nom', 'Description', 'Coach', 'Prix Mensuel', 'Prix Trimestriel', 'Prix Annuel', 'Capacite', 'Abonnes Actifs', 'Statut'], ';');
